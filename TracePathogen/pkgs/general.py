...
class MyRuleS():
    ...
    def run_multi(self):
        with Pool(int(self.parallel)) as ph:
            ph.map(run, self.lists_command_log)
...
def run(command_log:tuple):
    ...
    bin_current = os.path.dirname(_cml)
...
def merge_wgs_fasta(mfasta, dict_sample):
    ...
    for samp, spath in dict_sample.items():
        headnum = 1
        with open(spath) as fh:
            for line in fh:
                if line.startswith(">"):
                    newhead = f"{samp}_{headnum}"
