...
def get_args():
    ...
    parser.add_argument("-s", "--seq_stra", help="Sequencing strategy, such as "
    "[SE75, SE100, SE150, PE150, PE250...]", required=True)
    parser.add_argument("-o", "--outfq1", help="Output FASTQ, if --seq_type PE, FASTQ1.", required=True)
    parser.add_argument("-O", "--outfq2", help="Output FASTQ2, if --seq_type PE.")
    parser.add_argument("--min_cov", help="Min coverage depth. [default: 20]", default=20)
    parser.add_argument("--insert_len", help="Insert DNA length. If not provided, "
    "the program automatically predicts.")
...
class SimulateFQ():
    ...
    def __init__(self):
        ...
        self.seq_stra = self.args.seq_stra
    ...
    def assign_seq_strategy(self):
        ...
        if re.match(pattern, self.seq_stra):
            self.seq_type = self.seq_stra[:2]
            self.seq_length = int(self.seq_stra[2:])
        else:
            raise Exception(f"错误的测序策略! - {self.seq_stra}")
