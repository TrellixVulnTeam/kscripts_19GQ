...
#~ 超链接
FileLink($section, "新冠谱系分型结果查看请点击 <link=../cov_lineage/lineage_report_trans.xlsx>");
# 样本 SNP/InDel 变异结果
$section->submenu("样本 SNP/InDel 变异结果");
$section->desc("本分析流程的变异检测，是基于贝叶斯单倍型的遗传多态性算法。结果如下表所示：");
$section->tsv2html(
    -file   => "3.variants/trans.txt",
    -name   => "SNP/InDel 结果表",
    -header => 1);
FileLink($section, "变异文件查看请点击 <link=./3.variants/trans.xlsx>");
## 进化树分析
...
$section->img2html(
    ...
    -name => "矩形进化树图");
FileLink($section, "其他三种进化树图查看请点击 <link=./5.phylogenetic>");
### 基于SNPs方法构建进化树
$section->ssubmenu("基于SNPs方法构建进化树");
$section->desc("比对新冠病毒参考基因组获得各样本变异信息，多样本间共有 SNP 生成特有标记性序列，使用最大似然法构建进化树。");
$section->desc("使用 SNP-based 方法构建进化树相较于“基于多序列比对方法构建进化树”优势是在新冠病毒序列完整度不足时也可实现进化树的构建。");
if ( -e "src/source/snp_tree_rect.png" ){
    $section->desc("本报告提供同一种结果四种不同呈现方式的进化树，如下图所示：");
    $section->img2html(
        -file => "src/source/snp_tree_rect.png",
        -name => "矩形进化树图");
    FileLink($section, "其他三种进化树图查看请点击 <link=../snp_phylogenetic_tree>");
} else {
...
$section->img2html(
    ...
    -name => "矩形进化树图");
FileLink($section, "其他三种进化树图查看请点击 <link=../multiseq_phylogenetic_tree>");
