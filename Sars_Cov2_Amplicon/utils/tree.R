...
if (file.info(tree_file)$size == 0) {
  ...
  # create blank graph
  # cmdargs <- commandArgs()
  # progDirName <- dirname(gsub("--file=", "", cmdargs[4]))
  # blank_file <- paste0(progDirName,"/zhu/css_file/non_result_graph.png")
  # png_vector <- c("circular.png",
  #                 # "jagsaw.png",
  #                 "rectangular_none_branch_length.png",
  #                 "rectangular.png",
  #                 "slanted.png")
  # null <- lapply(png_vector, function(x)
  #   system(paste0("cp ",blank_file, " ", output_dir, "/", x), intern = T))
  quit(save = "no")
}
...
ggsave(plot = p1, filename = "rectangular_none_branch_length.png",
       width = p14_width, height = graph_height, dpi = 500, path = output_dir) #width = 9
...
ggsave(plot = p2, filename = "rectangular.png",
       width = p23_width, height = graph_height, dpi = 500, path = output_dir) #width = 9
#~ 4 slanted
p4 <- ggtree(tree, layout="slanted", branch.length = "none") +
  ...
  theme_tree2() +
  xlim(NA, ggtree_xlim_right)
ggsave(plot = p4, filename = "slanted.png",
       width = p14_width, height = graph_height, dpi = 500, path = output_dir) #width = 9
#~ 3 circular
p3 <- ggtree(tree, layout="circular",
  ...
  geom_tippoint(color="black", alpha=1/4) +
  geom_nodepoint(color="black", alpha=1/4)
ggsave(plot = p3, filename = "circular.png",
       width = p23_width, height = p23_width, dpi = 500, path = output_dir) #width = 9
#~ multi graphs
# pm <- plot_grid(p1, p2, p3, p4, ncol = 2, nrow = 2)
# ggsave(plot = pm, filename = "jagsaw.png",
#        width = pm_width, height = graph_height, dpi = 500, path = output_dir) #width = 15
